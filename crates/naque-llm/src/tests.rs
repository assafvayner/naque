...
async fn test_immediate_answer() {
    ...
    let result = agent.run_turn("hello", "", &mut executor).await.unwrap();
...
async fn test_provider_sees_prior_messages_on_second_turn() {
    ...
    let _ = agent.run_turn("q1", "", &mut executor).await.unwrap();
    ...
    let _ = agent.run_turn("q2", "", &mut executor).await.unwrap();
